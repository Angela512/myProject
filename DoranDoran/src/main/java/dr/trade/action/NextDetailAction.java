...
public class NextDetailAction implements Action{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int next=dao.getNext(trade_num);
		
		if(next!=0) {
			//조회수 증가
			dao.updateReadcount(next);
			
			//글 상세 정보 반환
			TradeVO trade=dao.getTrade(next);
		...
		return "/WEB-INF/views/trade/nextdetail.jsp";
	}
